...
if (debugBanner) debugBanner.innerHTML = '<div style="padding:20px;color:#94a3b8;font-size:14px;">ÔÅ│ A carregar formul├írio...</div>';
...
onAuthStateChanged(auth, async (user) => {
    ...
    if (debugBanner) debugBanner.innerHTML = `<div style="padding:20px;color:#94a3b8;font-size:13px;">­ƒæñ Conta: ${user.email}<br>­ƒöæ UID da conta: ${user.uid}<br>­ƒôï ID do formul├írio: ${currentFormId}<br>ÔÅ│ A verificar permiss├Áes...</div>`;
    ...
    try {
        ...
        if (formSnap.exists()) {
            ...
            if (debugBanner) debugBanner.innerHTML = `<div style="padding:20px;color:#94a3b8;font-size:13px;">­ƒôï Formul├írio carregado<br>­ƒæñ Conta: ${user.email}<br>­ƒöæ UID da conta: ${user.uid}<br>­ƒåö UID do form: ${formData.uid}<br>Ô£à ├ë dono: ${formData.uid === user.uid}<br>­ƒñØ Partilhado: ${(formData.sharedWith || []).includes(user.email)}</div>`;
            ...
            if (isOwner || isShared) {
                ...
                if (editorSchema.length === 0) {
                    ...
                    editorSchema = [{ type: 'title', content: currentForm.title || 'Novo Formul├írio' }];
                }
                ...
                // Form has a ghost/unrecognized UID ÔÇö offer to claim it inline (no popup)
                const formTitle = formData.schema?.[0]?.content || formData.title || 'Sem T├¡tulo';
                if (debugBanner) debugBanner.innerHTML = `
                    <div style="padding:30px;text-align:center;">
                        <p style="color:#94a3b8;margin-bottom:16px;">ÔÜá´©Å Este formul├írio parece ter um dono inv├ílido (UID fantasma).<br>Pode recuper├í-lo para a sua conta.</p>
                        <button onclick="claimThisForm('${currentFormId}')" style="background:#6366f1;color:white;border:none;padding:12px 24px;border-radius:8px;cursor:pointer;font-size:15px;">­ƒöæ Recuperar formul├írio para minha conta</button>
                ...
                window.claimThisForm = async (formId) => {
                    try {
                        ...
                        editorSchema = currentForm.schema || [];
                        if (editorSchema.length === 0) editorSchema = [{ type: 'title', content: currentForm.title || 'Novo Formul├írio' }];
                        renderCanvas();
                    } catch (claimErr) {
                        console.error("Failed to claim form:", claimErr);
                        if (debugBanner) debugBanner.innerHTML += `<p style="color:#f87171;padding:16px;">ÔØî Erro: ${claimErr.message}</p>`;
                    }
                };
            ...
            alert('Formul├írio n├úo encontrado.');
            window.location.href = 'dashboard.html';
        }
    } catch (error) {
        console.error("Erro a carregar formul├írio:", error);
        alert("Erro de comunica├º├úo com a base de dados: " + error.message);
    }
});
    ...
    editorSchema.forEach((block, index) => {
        ...
        if (block.type === 'title') {
            ...
            if (block.type === 'text-short' || block.type === 'text-long') {
                ...
                // Migration: 1D strings ÔåÆ 2D objects
                ...
                block.options.forEach((row, rowIdx) => {
                    // Gap zone BEFORE this row ÔÇö dropping here creates new row at rowIdx
                    ...
                    // The row itself ÔÇö catches drops that land in gap between pills
                        ...
                        html += `
                            <div class="pill-cell-wrapper" draggable="true" data-blockidx="${index}" data-rowidx="${rowIdx}" data-colidx="${colIdx}" ondragstart="handlePillDragStart(event)" ondragover="handlePillDragOver(event)" ondragleave="handlePillDragLeave(event)" ondrop="handlePillDrop(event)" ondragend="handlePillDragEnd(event)">
                                <div class="pill-edit-wrapper" style="${pillStyle}">
                                    ...
                                    <input type="text" class="pill-input" value="${optText}" data-blockidx="${index}" data-rowidx="${rowIdx}" data-colidx="${colIdx}" placeholder="Op├º├úo">
                                    <button class="pill-color-trigger" id="trigger-${index}-${rowIdx}-${colIdx}" title="Cor da p├¡lula" onclick="togglePillPalette(event, ${index}, ${rowIdx}, ${colIdx})" style="${iconStyle}">
                                        <i class="ph ph-palette"></i>
                                    </button>
                                    <button class="pill-delete-btn" title="Remover Op├º├úo" onclick="removeOption(${index}, ${rowIdx}, ${colIdx})" style="${iconStyle}">
                                        <i class="ph ph-x"></i>
                                    </button>
                                    <div id="palette-${index}-${rowIdx}-${colIdx}" class="pill-palette ${openPaletteId === `palette-${index}-${rowIdx}-${colIdx}` ? '' : 'hidden'}">
                                        <div class="palette-section">
                                            <div class="palette-label">Padr├úo</div>
                    ...
                    html += `<button class="tally-dashed-add" title="Adicionar p├¡lula nesta linha" onclick="addOptionToRow(${index}, ${rowIdx})"><i class="ph ph-plus"></i></button>`;
                    html += `</div>`; // close pill-row
                });

                // Final gap zone AFTER all rows ÔÇö dropping here adds a new row at the end
                html += `
        ...
        const toolsHtml = `
            <div class="block-tools-inline">
                ...
                <div class="tool-btn settings-btn" title="Defini├º├Áes"><i class="ph ph-gear"></i></div>
        ...
        blockEl.querySelector('.settings-btn').addEventListener('click', (e) => {
            ...
            if (block.type !== 'title' && block.type !== 'desc' && block.type !== 'section') {
                ...
                alert("Este bloco n├úo tem configura├º├Áes dispon├¡veis no momento.");
            }
        });
        ...
        // Attach listeners for pill inputs ÔÇö use .pill-input (current class)
        const pillInputs = blockEl.querySelectorAll('.pill-input');
        pillInputs.forEach(input => {
            input.addEventListener('blur', (e) => {
                // CRITICAL: skip if a pill drag is in progress ÔÇö the blur fires when drag
...
function appendNewBlock(type) {
    ...
    if (type === 'title-h1') {
        actualType = 'title';
        blockSize = 'large';
        initialContent = 'T├¡tulo Principal';
    } else if (type === 'title-h2') {
        actualType = 'section'; // In editor.js, H2 is 'section'
        blockSize = 'medium';
        initialQuestion = 'T├¡tulo M├®dio';
    } else if (type === 'title-h3') {
        actualType = 'section';
        blockSize = 'small';
        initialQuestion = 'T├¡tulo Pequeno';
    }
    ...
    if (actualType === 'choice-single' || actualType === 'choice-multi') {
        newBlock.options = [['Op├º├úo 1']];
    }
    if (actualType === 'counter') {
        newBlock.target = 0;
        newBlock.question = 'Novo Item de Invent├írio';
    }
...
settingDescriptionCheckbox.addEventListener('change', (e) => {
    if (activeSettingsIndex !== null) {
        ...
        if (e.target.checked && !editorSchema[activeSettingsIndex].description) {
            editorSchema[activeSettingsIndex].description = "Descri├º├úo opcional...";
        }
        renderCanvas();
        saveDebounce();
    }
...
// Row-level drag/drop fallback ÔÇö fires when drop lands in gap between pills.
...
window.handlePillDragOver = function (e) {
    ...
    // Determine zone ÔÇö only top/bottom 5px edge creates new row; rest is left/right same-row
    let zone;
    ...
    // Ghost pill ÔÇö pointer-events:none so drops pass through to the pill-row (which has ondrop)
...
window.addOption = function (blockIndex) {
    editorSchema[blockIndex].options.push([{ text: 'Nova Op├º├úo' }]);
    renderCanvas();
    saveDebounce();
};
...
window.addOptionToRow = function (blockIndex, rowIdx, colIdx) {
    ...
    row.splice(colIdx + 1, 0, { text: 'Op├º├úo', color: null });
    renderCanvas();
    saveDebounce();
};

window.addOptionRow = function (blockIndex, rowIdx) {
    editorSchema[blockIndex].options.splice(rowIdx + 1, 0, [{ text: 'Op├º├úo', color: null }]);
    renderCanvas();
    saveDebounce();
};
...
async function saveDebounce() {
    ...
    saveTimer = setTimeout(async () => {
        try {
            ...
            let formTitle = titleBlock ? (titleBlock.content || 'Novo Formul├írio') : 'Novo Formul├írio';
...
window.publishForm = async function (btn) {
    ...
    try {
        ...
        alert("Erro ao publicar o formul├írio. Veja a consola.");
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (btnChangeCover && coverUploadInput && coverBlock) {
        ...
        coverUploadInput.addEventListener('change', async (e) => {
            ...
            if (file.size > 5 * 1024 * 1024) {
                alert('A imagem n├úo pode exceder 5MB.');
                return;
            }
            if (!file.type.match('image.*')) {
                alert('Por favor selecione um ficheiro de imagem v├ílido.');
                return;
            }
            ...
            try {
                ...
                alert("Erro de liga├º├úo ao fazer upload. Verifica a tua internet.");
            } finally {
    ...
    if (btnPreview) {
        btnPreview.addEventListener('click', () => {
            if (!currentFormId) {
                alert('Formul├írio n├úo carregado ainda. Tente guardar primeiro.');
                return;
            }
        ...
        <div class="image-modal-content" onclick="event.stopPropagation()">
            ...
            <div class="image-modal-body">
                <div class="image-tab-bar">
                    ...
                    <button class="image-tab-btn" id="btn-tab-url" onclick="window.switchImageTab('url')"><i class="ph ph-link"></i> Endere├ºo Web (URL)</button>
                </div>
