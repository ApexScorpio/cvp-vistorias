def run():
    ...
    while i < len(lines):
        line = lines[i]
        if 'class="pill-palette ${openPaletteId === `palette-${index}-${rowIdx}-${colIdx}`' in line:
            ...
            # Skip the 3 divs
            i += 2
        elif 'class="pill-palette ${openPaletteId === `palette-inventoryItems-${index}-${rowIdx}-${iIdx}`' in line:
