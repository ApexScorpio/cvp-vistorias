...
let editorSchema = [];
Object.defineProperty(window, 'editorSchema', {
    get() { return editorSchema; },
    set(v) { editorSchema = v; }
});
...
Object.defineProperty(window, 'activeSettingsIndex', {
    get() { return activeSettingsIndex; },
    set(v) { activeSettingsIndex = v; }
});
...
// Saved colors are now managed globally by color-picker.js
    ...
    editorSchema.forEach((block, index) => {
                            ...
                            <div class="pill-cell-wrapper" id="cell-${index}-${rowIdx}-${iIdx}" style="width: auto; max-width: 280px;" ondragover="window.handlePillDragOver(event)">
                                ...
                                <div class="pill-edit-wrapper inventory-pill" 
                                    ...
                                    <div class="pill-actions-hover">
                                        ...
                                        <button class="pill-color-trigger color-trigger" title="Cor" id="trigger-inventoryItems-${index}-${rowIdx}-${iIdx}" onclick="window.openColorPicker(event, 'inventoryItems', ${index}, ${rowIdx}, ${iIdx})" style="${iconStyle}">
                        ...
                        html += `
                            ...
                            <div class="pill-cell-wrapper" draggable="true" data-blockidx="${index}" data-rowidx="${rowIdx}" data-colidx="${colIdx}" ondragstart="handlePillDragStart(event)" ondragover="handlePillDragOver(event)" ondragleave="handlePillDragLeave(event)" ondrop="handlePillDrop(event)" ondragend="handlePillDragEnd(event)" style="${cellWidth !== 'auto' ? `--pill-width: ${cellWidth}px; width: var(--pill-width);` : ''}">
                                ...
                                <div class="pill-edit-wrapper" style="${pillStyle}">
                                    ...
                                    <button class="pill-color-trigger color-trigger" id="trigger-pill-${index}-${rowIdx}-${colIdx}" title="Cor da pílula" onclick="window.openColorPicker(event, 'pill', ${index}, ${rowIdx}, ${colIdx})" style="${iconStyle}">
        ...
        blockEl.querySelector('.settings-btn').addEventListener('click', (e) => {
            ...
            if (block.type !== 'title' && block.type !== 'desc' && block.type !== 'section') {
                ...
                // Only show mapping filter toggle for answer-bearing blocks
                const filterableTypes = ['text-short', 'text-long', 'choice-single', 'choice-multi', 'date', 'counter'];
                if (filterableTypes.includes(block.type)) {
                    rowSettingFilter.style.display = 'flex';
                    settingFilterCheckbox.checked = !!editorSchema[index].useAsFilter;
                } else {
                    rowSettingFilter.style.display = 'none';
                }
...
settingFilterCheckbox.addEventListener('change', (e) => {
    ...
    if (activeSettingsIndex !== null) {
        ...
        editorSchema[activeSettingsIndex].useAsFilter = e.target.checked;
...
// Block settings color palette click listeners are bound inline in renderPaletteHTML
